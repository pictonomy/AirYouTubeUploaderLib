package com.nitobi.webapis.youtube
{
	...
	public class YouTubeService extends EventDispatcher
	{
		...
		public static const ALLOWED_EXTENSIONS:String = "wmv|avi|mov|mpg|flv|webm"; // note this must be lowercase
		...
		private function onUploadSuccess(evt:Event):void
		{
			...
			var successEvent:YouTubeEvent = new YouTubeEvent(YouTubeEvent.YT_UPLOAD_SUCCESS);
			successEvent.data = { videoId: _uploadAction.videoId, url: _uploadAction.url, file: _uploadAction.file };

			cleanup(_uploadAction);
		...
		private function cleanup(action:RestAction = null):void
		{
			if(_loginAction != null && (action == null || action == _loginAction))
			{
			...
			if(_logoutAction != null && (action == null || action == _logoutAction))
			{
			...
			if(_uploadAction != null&& (action == null || action == _uploadAction))
			{
			...
			if(_getUserVideosAction != null && (action == null || action == _getUserVideosAction))
			{
			...
			if(_getUploadTokenAction != null && (action == null || action == _getUploadTokenAction))
			{
